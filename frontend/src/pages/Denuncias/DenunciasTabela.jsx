          ...
          {denuncias.map(d => {
            ...
            return (
              <tr key={d.id} className="dn-row" onClick={() => onAbrir(d)}>
                ...
                <td className="dn-price">{brl(d.precoValor)}</td>
                <td><span className={`dn-badge ${badge.cls}`}>{badge.label}</span></td>
                <td className="dn-meta">👍 {d.votosConfirma} · 👎 {d.votosRejeita}</td>
              </tr>
            )
          })}
        </tbody>
